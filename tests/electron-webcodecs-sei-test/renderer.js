...
const el = {
  ...
  sensorEpochTs: document.getElementById('sensorEpochTs'),
...
function applySignalFromQuery() {
  ...
  if (fromQuery) el.signal.value = fromQuery;
}
...
function setDecoderStatus(text, cls) {
  el.decoderStatus.textContent = text;
  el.decoderStatus.className = cls || '';
}

function parseSensorSize() {
  const m = (el.sensorSize.value || '').trim().match(/^(\d+)x(\d+)$/i);
  if (!m) throw new Error('Invalid sensor size format, expected WxH (e.g. 4056x3040)');
...
function readCropFromSei(meta) {
  if (!Number.isFinite(meta.x) || !Number.isFinite(meta.y) || !Number.isFinite(meta.width) || !Number.isFinite(meta.height)) return;
...
function findAnnexBNalus(data) {
  ...
  while (i + 3 < data.length) {
    ...
    while (j + 3 < data.length) {
      if (data[j] === 0 && data[j + 1] === 0 && (data[j + 2] === 1 || (j + 3 < data.length && data[j + 2] === 0 && data[j + 3] === 1))) break;
      j++;
    }
...
function parseSeiFromFrame(frameData) {
  ...
  for (const nalu of nalus) {
    ...
    while (off + 1 < rbsp.length) {
      ...
      if (payloadType !== 5 || payload.length < 68) continue;
      ...
      return {
        frameId: readU64BE(msg, 4),
        x: readU32BE(msg, 12) >> 0,
        y: readU32BE(msg, 16) >> 0,
        width: readU32BE(msg, 20),
        height: readU32BE(msg, 24),
        captureTsUs: readU64BE(msg, 28),
        sensorTsUs: readU64BE(msg, 36),
        sensorEpochUs: readU64BE(msg, 44)
      };
    }
  }
...
function onMetadata(meta) {
  ...
  el.sensorEpochTs.textContent = String(meta.sensorEpochUs);
  el.updatedAt.textContent = new Date().toLocaleTimeString();
  readCropFromSei(meta);
}

function addMetadata(timestamp, meta) {
  metadataByTs.set(timestamp, meta);
  while (metadataByTs.size > MAX_META_ENTRIES) {
    const firstKey = metadataByTs.keys().next().value;
    metadataByTs.delete(firstKey);
  }
}

function takeMetadata(timestamp) {
  if (metadataByTs.has(timestamp)) {
    const meta = metadataByTs.get(timestamp);
    metadataByTs.delete(timestamp);
    return meta;
  }

  let bestKey = null;
  let bestDiff = Number.POSITIVE_INFINITY;
  for (const key of metadataByTs.keys()) {
  ...
  if (bestKey !== null && bestDiff <= 5000) {
    const meta = metadataByTs.get(bestKey);
    metadataByTs.delete(bestKey);
    return meta;
  }
...
function handleDecodedFrame(frame) {
  try {
    ensureCanvasSize(frame.displayWidth || frame.codedWidth, frame.displayHeight || frame.codedHeight);
    ctx.drawImage(frame, 0, 0, el.canvas.width, el.canvas.height);

    rendered += 1;
    decoded += 1;
    el.renderedFrames.textContent = String(rendered);
    el.decodedFrames.textContent = String(decoded);

    const ts = Number(frame.timestamp || 0);
    const meta = takeMetadata(ts);
...
async function configureDecoderFromMetadata(metadata) {
  if (!metadata || !metadata.decoderConfig) return false;
  const cfg = metadata.decoderConfig;
  if (!cfg.codec) return false;

  if (!decoder) {
    decoder = new VideoDecoder({
      output: handleDecodedFrame,
      error: (e) => setDecoderStatus('error: ' + e.message, 'err')
    });
  }
  ...
  if (decoder.state !== 'configured') {
    ...
    setDecoderStatus(`configured (${cfg.codec})`, 'ok');
  }
...
function ensureFallbackDecoder() {
  ...
  if (decoder.state !== 'configured') {
    ...
    setDecoderStatus('configured fallback (avc1.42E01E)', 'warn');
  }
}
...
function setupEncodedTransform(receiver) {
  ...
  const ts = new TransformStream({
    async transform(frame, controller) {
      ...
      const tsUs = Number(frame.timestamp || 0);
      try {
        const sei = parseSeiFromFrame(frame.data);
        if (sei) addMetadata(tsUs, sei);
      } catch (e) {
        console.log('SEI parse error:', e);
      }

      try {
        const frameMeta = typeof frame.getMetadata === 'function' ? frame.getMetadata() : null;
        const configured = await configureDecoderFromMetadata(frameMeta);
        if (!configured) ensureFallbackDecoder();
        ...
        setDecoderStatus('decode failed: ' + e.message, 'err');
      }
...
async function connect() {
  ...
  el.decodedFrames.textContent = '0';
  el.seiCount.textContent = '0';
  setDecoderStatus('initializing', 'warn');
  ...
  pc.addEventListener('track', (evt) => {
    if (evt.receiver) setupEncodedTransform(evt.receiver);
