...
static rtc::binary prepend_sei_metadata(const rtc::binary &h264, uint64_t frame_id, buffer_t *buf)
{
  ...
  // Metadata schema v3:
  // magic[4]="CSM1", frame_id(u64), x(i32), y(i32), width(u32), height(u32),
  // capture_ts_us(u64), sensor_ts_us(u64), sensor_epoch_us(u64)
  uint8_t payload[4 + 8 + 4 + 4 + 4 + 4 + 8 + 8 + 8] = {0};
  payload[0] = 'C';
  payload[1] = 'S';
  payload[2] = 'M';
  payload[3] = '1';
  ...
  if (realtime_now_us > 0 && boottime_now_us > 0 && sensor_ts_us > 0) {
    // Convert BOOTTIME-based sensor timestamp to CLOCK_REALTIME epoch domain.
